...
void SuperTwisting::_staZPositionControl(const float dt, matrix::Vector3f& _pos, matrix::Vector3f& _pos_sp, matrix::Vector3f& _vel)
{
	...
	if(!PX4_ISFINITE(deri_z_pos)){
	 ...
	 _last_z_pos = _pos(2);
	 float ita = deri_z_pos + _sta_sliding_c * _sta_int_z_pos_error;
	 ...
	 _sta_thrust = _sta_sliding_c * deri_z_pos + _sta_lamada * ita / sqrt(ita_norm) + _sta_w;
